...
\subsection*{Probleme}
\chapterauthor{Maximilian Bertram}
\subsubsection*{Objekte greifen in einer Bewegung}
Es war schwierig das Greifen der Objekte in einer einzigen Bewegung zu planen.
\textbf{Lösung}: Wir haben uns dazu entschieden die Bewegung für das Greifen und Abstellen auf mehrere Male auszuführen. Zuerst wird der gewünschte Gripper geöffnet und vor das Objekt gefahren. Dann wird der geöffnete Gripper nur auf einer Koordinatenachse bewegt und so \textit{über} das Objekt geschoben.
...
\paragraph{1.2.1.1 Offered Actions}
...
\paragraph{1.2.1.3 Called Services}
\chapterauthor{RH, MB}
...
Wird ein Objekt erfolgreich abgestellt oder gegriffen werden für den Beliefstate von Knowledge noch die entsprechenden Messages gepublished: \\

\begin{lstlisting}[caption={Definition des GraspObject.msg},captionpos=b]
...
\begin{lstlisting}[caption={Definition des DropObject.msg},captionpos=b]
#A message which get's published by Motion-component after dropping an object.
#The gripper used. See 'Gripper.msg' for more details.
Gripper gripper
\end{lstlisting}

\begin{lstlisting}[caption={Definition des Gripper.msg},captionpos=b]
...
\paragraph{1.2.2.1 Offered Actions}
...
#bool finished
\end{lstlisting}
\paragraph{1.2.2.2 Called Actions}
