...
Wurde ein Objekt abgestellt, wird dem place\_object-Knoten über dieses Topic mitgeteilt, dass die Vorschau entfernt werden kann.

\subsubsection{Publisher spawn\_attached\_object}
Auf diesem Topic published der Beliefstate, falls ein Objekt mithilfe eines Menschen gegriffen wurde.

\subsubsection{Publisher perceived\_object\_bounding\_box}
Wenn ein Objekt wahrgenommen wurde, wird über dieses Topic die Pose ein und das entsprechende CAD-Model an Motion weitergereicht

\subsection{Programmablauf}
\chapterauthor{Alexander Haar}
\subsubsection{Schritt 1: Wahrnehmen mehrerer Objekte}
Es werden mehrere Objekte wahrgenommen und deren Label sowie Pose wird dem Beliefstate bekannt gemacht.
...
Es wird anschließend das Objekt-Mesh gespawnt, so wie das Objekt perfekt abgestellt aussehen sollte.
