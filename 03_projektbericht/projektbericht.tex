...
\subsection{Ablauf}
Für den Gesamtablauf siehe bitte Kapitel: \ref{main:1}.
...
\begin{figure}[!htb]
        ...
        {img/architekturbild_gesamt.png}
...
\section{main.lisp (planning\_main\_programm)}
...
\begin{figure}[!htb]
        ...
        \caption{} Main-Diagramm von Vanessa}
\end{figure}
...
\newpage
\section{vision\_communicate.lisp (planning\_vision)}
...
\section{knowledge-communicate.lisp\\
planning\_knowledge)}
...
Gibt die Position, Länge und Breite des Storage-places eines Objekts zurück.\\

2. 'knowledge\_grasp/knowledge\_grasp'\\
Gibt eine Grasppose für ein Objekt zurück.\\

3. 'place\_object/place'\\
Gibt eine Pose zum Abstellen von Objekten zurück.\\

4. 'push\_object/push'\\
Gibt eine Verschiebepose für ein Objekt zurück.\\

5. '/svm\_classifier/classify\_service'\\
Identifiziert ein Objekt anhand von Features.\\

6. 'beliefstate/objects\_to\_pick'\\
...
7. 'beliefstate/gripper\_empty'\\
Wir können erfragen, welcher Gripper frei ist.\\

8. 'beliefstate/objects\_attached\_to\_gripper\\
Gibt den Objektnamen des Objektes zurück, welches sich in der gegebenen Hand befindet.\\
...
\subsubsection{is-object()}
\chapterauthor{Hauke Tietjen}
\begin{verbatim}
is-object()

Beschreibung: Fragt Knowleedge ob die gegebenen Features zu dem
gegeben Objekt (String) passen.

@param: objectString features
@return: T wenn die Features zum Objekt passen, nil falls nicht.
\end{verbatim}

\subsubsection{ask-knowledge-how-to-grab()}
\chapterauthor{Hauke Tietjen}
\begin{verbatim}
ask-knowledge-how-to-grab()

Beschreibung: Fragt Knowledge wie das Objekt gegriffen werden soll.
Knowledge entscheidet dann von welcher Seite und gibt eine Pose zurück.

@param: object
@return: Greifpose
\end{verbatim}

\subsubsection{ask-knowledge-where-belongs()}
\chapterauthor{Hauke Tietjen}
\begin{verbatim}
ask-knowledge-where-belongs()
...
\subsubsection{place-object()}
\chapterauthor{Hauke Tietjen}
\begin{verbatim}
place-object()

Beschreibung: Fragt Knowledge wie das Objekt, mit dem besagtem Arm,
an der gegebenen Position abgestellt werden soll. Knowledge gibt
daraufhin eine Pose zrück.

@param: gripper frame-id x-coordinate y-coordinate
@return: Abstellpose
\end{verbatim}

\subsubsection{objects-to-pick()}
\chapterauthor{Vanessa Hassouna}
\begin{verbatim}
objects-to-pick()

Beschreibung: Gibt uns zwei Strings zurück. Diese sind leer, wenn kein
Objekt mehr gegriffen werden soll oder es kein Objekt mehr zum Greifen gibt.
...
gegriffen werden soll. Knowledge entscheidet ob von oben, links oder rechts
gegriffen wird.
...
\subsubsection{empty-gripper()}
\chapterauthor{Hauke Tietjen}
\begin{verbatim}
empty-gripper()

Beschreibung: Fragt Knowledge welcher Gripper frei ist.
Knowledge gibt darauf eine Message zurück, die boolische
Werte für beide Gripper enthält.

@param:
@return: Knowledge Message
\end{verbatim}

\subsubsection{push-objects()}
\chapterauthor{Hauke Tietjen}
\begin{verbatim}
pusch-objects()

Beschreibung: Fragt Knowledge nach einer Verschiebepose, für das
gegebene Objekt.

@param: object-label
@return: Verschiebepose
\end{verbatim}

\subsubsection{get-object-attached-to-gripper()}
\chapterauthor{Hauke Tietjen}
\begin{verbatim}
get-object-attached-to-gripper()

Beschreibung: Fragt Knowledge welches Objekt sich in der gegebenen
Hand befindet.

@param: gripper-number
@return: object-lable
\end{verbatim}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{motion-actions.lisp (planning\_motion)}
\subsection{Architekturbild}
\chapterauthor{Vanessa Hassouna}
...
\newpage
\section{movement.lisp (planning\_move)}
\subsection{Architekturbild}
\chapterauthor{Kevin Störmer}
...
Beschreibung: Anhand des Actionservice "nav_pcontroller_move_base" 
wird die Basis bewegt. Der Punkt und die Rotation sind in dem Frame 
"map" anzugeben.
...
\section{external-logic.lisp (planning\_logic)}
\subsection{Architekturbild}
\chapterauthor{Vanessa Hassouna}
...
\chapterauthor{Vanessa Hassouna}
\begin{verbatim}
transformation-Vision-Point (pose amount &optional (endFrame "/base_footprint")
...
Beschreibung: Transformiert einen Punkt mit dem standard
Quaternion  0 0 0 1.
...
Beschreibung: Ruft vision_suturo Service um Objekte wahrzunehmen auf.
Währenddessen wird durch die Größe des übergebenen Arrays
durchgelaufen und jedes gesehene Objekt gepublisht, sowie
auf dem Paramserver gespeichert. Auch ein Counter wird gesetzt,
der mitzählt wie viele Objekte gesehen wurden.
...
Beschreibung: Noch einmal wird vision_suturo Service aufgerufen. Nun werden
alles Objekte mit dem übergebenem Label verglichen. 
...
Der Roboter selber bewegt sich in dem Frame base_footprint.
...
Beschreibung: Die Funktion versucht zu greifen oder abzustellen. Wenn der
...
Beschreibung: Die Funktion ruft grab-or-place-object auf. Wenn der
Counter größer ist als 0 verändert der Pr2 seine Position
...
und setzt die Variablen für den linken und rechten Arm auf
T oder Nil.
...
\newpage
\section{objects.lisp (planning\_objects)}
\subsection{Architekturbild}
\chapterauthor{Hauke Tietjen}
...
\section{error-hierarchy.lisp (planning\_error)}
\subsection{Architekturbild}
\chapterauthor{Hauke Tietjen}
\begin{figure}[!htb]
	\center{\includegraphics[width=0.4\textwidth]
...
\section{interaction.lisp (planning\_interaction)}
\subsection{Architekturbild}
\chapterauthor{Kevin Störmer}
...
\subsubsection{\"Ubersicht}
\chapterauthor{Kevin Störmer}
Die Datei interaction.lisp ist zuständig dafür, Situationen aufzulösen, die nicht vom Code abgedeckt werden, oder die für den Roboter physikalisch nicht möglich sind.\\
...
Beschreibung: Nutzt das sound_play Paket um einen String in 
...
\section{old.lisp (planning\_old)}
...
\subsubsection{Beschreibung des Teilsystems}
\chapterauthor{Max-Phillip Bahr}
...
\item[3.]Die entsprechende Prolog Methode liest die zum übergebenen Objektlabel gehörigen 

GraspPosen aus der OWL Ontologie aus.
\item[4.] Die als PrologBindings vom json\_prolog- Server erhaltenen GraspPosen werden von 

find\_grasp\_pose (c++) in die gewünschte Form konvertiert.
...
Des Weiteren ist der Beliefstate in die Mensch-Roboter-Interaktion involviert. Falls ein Objekt nicht gegriffen werden kann, wird der Beliefstate so manipuliert, dass ein Zustand hergestellt wird, in dem das Objekt von oben gegriffen wurde. Sollte ein Objekt nicht abgestellt werden können, wird es komplett aus dem Beliefstate entfernt. 
...
Publisher despawn\_place\_preview: Wurde ein Objekt abgestellt, wird dem place\_object-Knoten über dieses Topic mitgeteilt, dass die Vorschau entfernt werden kann.\\

Publisher spawn\_attached\_object: Auf diesem Topic published der Beliefstate, falls ein Objekt mithilfe eines Menschen gegriffen wurde.\\

Publisher perceived\_object\_bounding\_box: Wenn ein Objekt wahrgenommen wurde, wird über dieses Topic die Pose und das entsprechende CAD-Model an Motion weitergereicht

\subsubsection{Programmablauf}
\chapterauthor{Alexander Haar}
\begin{itemize}
\item[1.]Es werden mehrere Objekte wahrgenommen und deren Label sowie Pose wird dem Beliefstate bekannt gemacht.
\item[2.]Nachdem alle zu verteilenden Objekte bekannt sind, werden möglichst zwei Objekte ausgewählt, welche den gleichen Lagerplatz besitzen.
\item[3.]Das erste der beiden Objekte wird gegriffen.
\item[4.]Anschließend wird das zweite Objekt gegriffen.
\item[5.]Der Roboter fährt zum entsprechenden Lagerplatz und stellt es dort ab.
\item[6.]Nun wird geprüft, ob beide Gripper leer sind, dies ist nicht der Fall.
\item[7.]Der Roboter fährt zum nächsten Lagerplatz und stellt das zweite Objekt dort ab.
\item[8.]Erneut wird geprüft, ob beide Gripper leer sind, dies ist jetzt der Fall.
\item[9.]Nun beginnt dieser Prozess wieder bei Schritt 2.
\end{itemize}
...
\chapterauthor{Alexander Haar}
\begin{itemize}
\item[1.]Eine Objektklasse wird an den Service- Server übergeben.
\item[2.]Anhand der Objektklasse wird eine Prologanfrage erstellt und an den json\_prolog- Server geschickt.
\item[3.]Durch die oben definierten Prologregeln wird unter Verwendung von only- restrictions der Lagerplatz ermittelt.
\item[4.]Die vom json\_prolog- Server erhaltene Antwort wird entsprechend geparst und zurückgegeben.
\end{itemize}

\subsection{push\_object}
\chapterauthor{Alexander Haar}
Das push\_object-Paket errechnet einen Punkt auf der Oberfläche eines abgestellten Objektes, damit es nach hinten geschoben werden kann. Dabei wird ausgenutzt, dass die Objekte immer gleich abgestellt werden. Bei den runden Objekten ist es für das Schieben unerheblich wie sie abgestellt werden, bei den eckigen Objekte zeigt aber immer die kurze Seite zum Roboter. Da man die Position des Objektes im Raum kennt und die geometrischen Abmessungen kann man nun eine passende Schiebepose berechnen.
...
\item[1.]Eine Objektklasse wird an den Service- Server übergeben.
\item[2.]Anhand der Objektklasse wird eine Prologanfrage erstellt und an den json\_prolog- Server geschickt.
\item[3.]Es wird mithilfe von Prolog eine Schiebepose errechnet. Das genaue Vorgehen ist oben in den einleitenden Worten beschrieben.
\item[4.]Die vom json\_prolog- Server erhaltene Antwort wird entsprechend geparst und zurückgegeben.
\end{itemize}

\subsection{place\_object}
\chapterauthor{Alexander Haar}
...
\begin{itemize}
\item[1.]Ein Gripper wird an den Service- Server übergeben.
\item[2.]Anhand des Grippers wird eine Prologanfrage erstellt und an den json\_prolog- Server geschickt.
\item[3.]Es wird mithilfe von Prolog eine Abstellpose errechnet. Das genaue Vorgehen ist oben in den einleitenden Worten beschrieben.
\item[4.]Die vom json\_prolog- Server erhaltene Antwort wird entsprechend geparst und zurückgegeben.
\item[5.]Es wird anschließend das Objekt-Mesh gespawnt, so wie das Objekt perfekt abgestellt aussehen sollte.
\end{itemize}
...
\begin{itemize}
\item[1.]Es wird eine Anfrage an den Service- Server gestellt.
\item[2.]Es wird eine Prologanfrage erstellt und an den json\_prolog- Server geschickt.
\item[3.]Mithilfe von Prolog werden alle Küchenobjekte ermittelt.
\item[4.]Die vom json\_prolog- Server erhaltene Antwort wird entsprechend geparst und zurückgegeben.
\end{itemize}
...
\chapterauthor{Roman Haak}
Die Untergruppe '\textit{Motion}' ist für das Bewegen der verschiedenen Gelenke des Roboters zwecks Manipulation der Umwelt zuständig.\\
...
Um einen reibungslosen Entwicklungsprozess zu gewährleisten, war es nötig, unsere Library durch Unit-Tests abzudecken. Dies stellte uns vor diverse Herausforderungen. Zum einen lässt sich ROS-Funktionalität nicht testen; dadurch ist es nötig, die Core-Funktionalität unserer Library von ROS Funktionen zu befreien. Ein erster Ansatz war es, hierfür die \textit{ROSConnector-Klasse} anzulegen, welche einen Wrapper um ROS Methoden darstellt und lediglich Methoden anbietet, die den Aufruf weiter an ROS delegieren. Dies sollte es ermöglichen, die nötigen ROS Aufrufe in unserer Library mit einem Mocking-Framework (in unserem Fall gMock) zu mocken und so Unit-Tests fahren zu können. Diesen Ansatz haben wir verfolgt, um dann leider festzustellen, dass gMock noch nicht von \textit{catkin} unterstützt wird. gMock ist nur in einer \textit{forked catkin version} verfügbar und es ist wohl geplant, das Framework in einer neueren Version von \textit{catkin} in das Main-Release zu integrieren. Daher haben wir uns dazu entschlossen, unsere Library soweit vorzubereiten, damit die \textit{Unit-Tests} nach dem Release und der Migration auf die neueste Version keinen großen Aufwand darstellen. \textit{Catkin build} führt beim Buildprozess keine Tests aus, diese können nur über \textit{catkin\_make run\_tests} ausgeführt werden. Damit die Tests dennoch regelmäßig laufen und die Funktionalität der Lib regelmäßig getestet wird, wurde \textit{Continuous Integration} eingerichtet. Über den Dienst \textit{coverage.io} lässt sich zusätzlich ein \textit{Coveragereport} für die getesteten Packages erstellen, dadurch ist es möglich, Qualitätsstandards zu definieren und \textit{Mergerequests} nur bei ausreichender Testabdeckung zu akzeptieren. Leider war es uns nicht möglich, dies hier umzusetzen, da die OpenSource Registrierung von coverage.io zum Zeitpunkt der Umsetzung nicht funktionierte. \\
...
Um den Entwicklungsprozess weiter zu verbessern haben wir uns dazu entschieden, \textit{Continuous Integration} zu verwenden, um eine reibungslose Integration der Feature-Branches in den master-Branch zu gewährleisten. Dabei haben wir den CI-Dienst von \textit{Travis} genutzen. Einer der Hauptgründe hierfür ist die verhältnismäßig leichte Konfiguration über die \textit{.travis.yml} YAML-Datei. Dabei musste \textit{catkin\_make} verwendet werden, da \textit{catkin build} generell keine Tests ausführt. Durch die \textit{private dependencies} unserer Node war die Einrichtung von Travis nicht trivial. Diese mussten zuerst gecloned und gebaut werden, um im lokalen Buildrepository zur Verfügung zu stehen. Am besten lässt sich dies durch folgende kommentierte \textit{.travis.yml} zeigen. Relevant sind hier vor Allem die Bereiche \textit{install}, \textit{before\_script} und \textit{script}: \\
...
\textit{before\_script section of .travis.yml}
